define(['svg', 'config', 'force', 'jquery', 'people', 'node', 'sio' ],
    function ( svgModule, config, layoutModule, $, peopleModule, nodeModule, sio ){
        var container = svgModule.init('.svg');
        layoutModule.makeForce(container);
        layoutModule.start();

        var screen = sio.connect( '/screen', {
        ...
        function generateFlow( data ){
            var people = peopleModule.getPeople( (data)?data:null );
            people.forEach( function( person ){
                layoutModule.addNode( person.trim() );
                layoutModule.start();
            });

            var owner = nodeModule.findNode(peopleModule.getName( data.centerName ), layoutModule);
            var friends = peopleModule.getFriends( data.friends );
